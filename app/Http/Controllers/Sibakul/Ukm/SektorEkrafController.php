...
use App\Models\Sibakul\Ukm\SektorEkraf;
...
class SektorEkrafController extends Controller
{
    public function index()
    {
        ...
        ->post('https://sibakuljogja.jogjaprov.go.id/restapi/api', [
            'data' => 'datapublik_UKM_Per_Sektor_Ekraf',
            'limit' => '0,100'
        ]);
    
        $data = $response->json();

        $key='datapublik_UKM_Per_Sektor_Ekraf';
        
        foreach ($data[$key] as $value ) {

            SektorEkraf::updateOrCreate([
                'sektor_ekraf'=> $value['sektor_ekraf']
            ],
            [ 
                'sektor_ekraf'=> $value['sektor_ekraf'],
                'btl' => $value['BTL'],
