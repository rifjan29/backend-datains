...
use App\Models\Sibakul\Ukm\SektorGroup;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Http;

class SektorGroupController extends Controller
{
    public function index()
    {
        ...
        ->post('https://sibakuljogja.jogjaprov.go.id/restapi/api', [
            'data' => 'datapublik_UKM_Per_Sektor_Pergup',
            'limit' => '0,100'
        ]);
    
        $data = $response->json();

        $key='datapublik_UKM_Per_Sektor_Pergup';
        
        foreach ($data[$key] as $value ) {

            SektorGroup::updateOrCreate([
                'sektor_pergub'=> $value['sektor_pergub']
            ],
            [ 
                'sektor_pergub'=> $value['sektor_pergub'],
                'btl' => $value['BTL'],
