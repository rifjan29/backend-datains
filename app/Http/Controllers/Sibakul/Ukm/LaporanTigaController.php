...
use App\Models\Sibakul\Ukm\LaporanTiga;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Http;

class LaporanTigaController extends Controller
{
    public function index()
    {
        ...
        ->post('https://sibakuljogja.jogjaprov.go.id/restapi/api', [
            'data' => 'datapublik_Laporan_UKM_3',
            'limit' => '0,100'
        ]);
    
        $data = $response->json();

        $key='datapublik_Laporan_UKM_3';
        
        foreach ($data[$key] as $value ) {

            LaporanTiga::updateOrCreate([
                'uraian'=> $value['uraian']
            ],
            [ 
                'uraian'=> $value['uraian'],
