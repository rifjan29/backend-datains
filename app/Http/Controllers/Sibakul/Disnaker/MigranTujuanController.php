...
use App\Models\Sibakul\Disnaker\MigranTujuan;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Http;

class MigranTujuanController extends Controller
{
    public function index()
    {
        ...
        ->post('https://sibakuljogja.jogjaprov.go.id/restapi/api', [
            'data' => 'disnaker_trans_idmc_Laporan_Transmigrasi_By_Tujuan',
            'limit' => '0,100'
        ]);
    
        $data = $response->json();

        $key='disnaker_trans_idmc_Laporan_Transmigrasi_By_Tujuan';
        
        foreach ($data[$key] as $value ) {

            MigranTujuan::updateOrCreate([
                'tahun'=> $value['tahun'],
                'kab_prov' => $value['kab_prov']
            ],
            [
                'jumlah_kk' => $value['jumlah_kk'],
                'jumlah_jiwa' => $value['jumlah_jiwa'],
                'tahun' => $value['tahun'],
                'kab_prov' => $value['kab_prov'],
                'created_at' => now(),
                'updated_at' => now(),
