...
use App\Models\Sibakul\Disnaker\AngkatanKerjaUmur;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Http;

class AngkatanKerjaUmurController extends Controller
{
    public function index()
    {
        ...
        ->post('https://sibakuljogja.jogjaprov.go.id/restapi/api', [
            'data' => 'disnaker_trans_idmc_Angakatan_Kerja_Berdasarkan_Umur',
            'limit' => '0,100'
        ]);
    
        $data = $response->json();

        $key='disnaker_trans_idmc_Angakatan_Kerja_Berdasarkan_Umur';
        
        foreach ($data[$key] as $value ) {

            AngkatanKerjaUmur::updateOrCreate([
                'tahun'=> $value['Tahun'],
                'kelompok_umur' => $value['Kelompok_Umur']
            ],
            [
                'jumlah' => $value['Jumlah'],
                'laki' => $value['Laki'],
