...
use App\Models\Sibakul\Disnaker\MigranAsalTujuan;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Http;

class MigranAsalTujuanController extends Controller
{
    public function index()
    {
        ...
        ->post('https://sibakuljogja.jogjaprov.go.id/restapi/api', [
            'data' => 'disnaker_trans_idmc_Laporan_Transmigrasi_By_Asal',
            'limit' => '0,100'
        ]);
    
        $data = $response->json();

        $key='disnaker_trans_idmc_Laporan_Transmigrasi_By_Asal';
        
        foreach ($data[$key] as $value ) {

            MigranAsalTujuan::updateOrCreate([
                'tahun'=> $value['tahun'],
