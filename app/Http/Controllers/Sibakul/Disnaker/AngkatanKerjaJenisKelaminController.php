...
use App\Models\Sibakul\Disnaker\AngkatanKerjaJenisKelamin;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Http;

class AngkatanKerjaJenisKelaminController extends Controller
{
    public function index()
    {
        ...
        ->post('https://sibakuljogja.jogjaprov.go.id/restapi/api', [
            'data' => 'disnaker_trans_idmc_Angakatan_Kerja_Berdasarkan_Jenis_Kelamin',
            'limit' => '0,100'
        ]);
    
        $data = $response->json();

        $key='disnaker_trans_idmc_Angakatan_Kerja_Berdasarkan_Jenis_Kelamin';
        
        foreach ($data[$key] as $value ) {

            AngkatanKerjaJenisKelamin::updateOrCreate([
                'tahun'=> $value['tahun'],
