...
use App\Models\Sibakul\Disnaker\PengangguranJenisKelamin;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Http;

class PengangguranJenisKelaminController extends Controller
{
    public function index()
    {
        ...
        ->post('https://sibakuljogja.jogjaprov.go.id/restapi/api', [
            'data' => 'disnaker_trans_idmc_Data_Penganggur_Berdasarkan_Jenis_Kelamin',
            'limit' => '0,100'
        ]);
    
        $data = $response->json();

        $key='disnaker_trans_idmc_Data_Penganggur_Berdasarkan_Jenis_Kelamin';
        
        foreach ($data[$key] as $value ) {

            PengangguranJenisKelamin::updateOrCreate([
                'tahun'=> $value['tahun'],
