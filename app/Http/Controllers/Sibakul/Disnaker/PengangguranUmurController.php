...
use App\Models\Sibakul\Disnaker\PengangguranUmur;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Http;

class PengangguranUmurController extends Controller
{
    public function index()
    {
        ...
        ->post('https://sibakuljogja.jogjaprov.go.id/restapi/api', [
            'data' => 'disnaker_trans_idmc_Data_Penganggur_Berdasarkan_Umur',
            'limit' => '0,100'
        ]);
    
        $data = $response->json();

        $key='disnaker_trans_idmc_Data_Penganggur_Berdasarkan_Umur';
        
        foreach ($data[$key] as $value ) {

            PengangguranUmur::updateOrCreate([
                'tahun'=> $value['Tahun'],
