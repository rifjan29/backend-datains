...
use App\Models\Sibakul\Disnaker\AngkatanKerjaPendidikan;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Http;

class AngkatanKerjaPendidikanController extends Controller
{
    public function index()
    {
        ...
        ->post('https://sibakuljogja.jogjaprov.go.id/restapi/api', [
            'data' => 'disnaker_trans_idmc_Angakatan_Kerja_Berdasarkan_Pendidikan',
            'limit' => '0,100'
        ]);
    
        $data = $response->json();

        $key='disnaker_trans_idmc_Angakatan_Kerja_Berdasarkan_Pendidikan';
        
        foreach ($data[$key] as $value ) {

            AngkatanKerjaPendidikan::updateOrCreate([
                'tahun'=> $value['Tahun'],
