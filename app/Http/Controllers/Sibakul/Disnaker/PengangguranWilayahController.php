...
use App\Models\Sibakul\Disnaker\PengangguranWilayah;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Http;

class PengangguranWilayahController extends Controller
{
    public function index()
    {
        ...
        ->post('https://sibakuljogja.jogjaprov.go.id/restapi/api', [
            'data' => 'disnaker_trans_idmc_Data_Penganggur_Berdasarkan_Wilayah',
            'limit' => '0,100'
        ]);
    
        $data = $response->json();

        $key='disnaker_trans_idmc_Data_Penganggur_Berdasarkan_Wilayah';
        
        foreach ($data[$key] as $value ) {

            PengangguranWilayah::updateOrCreate([
                'tahun'=> $value['Tahun'],
